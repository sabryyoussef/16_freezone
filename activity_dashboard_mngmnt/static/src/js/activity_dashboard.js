...
render_dashboards: function () {
    var self = this;
    var isAdmin = session.user_has_group('activity_dashboard_mngmnt.activity_dashboard_admin_group');
    var uid = session.uid;
    var countDomain = isAdmin ? [] : [['user_id', '=', uid]];

    this._rpc({
        model: 'mail.activity',
        method: 'get_activity_count',
        args: [countDomain]  // Fixed: remove extra array nesting
    ...
    function buildDomain(state) {
        var domain = [["state", "=", state]];
        if (!isAdmin) {
    ...
    self._rpc({
        model: 'mail.activity',
        method: 'search_read',
        domain: buildDomain('done'),
        context: { active_test: false },
    }).then(function (done_activity) {
        self._rpc({
            model: 'mail.activity',
            method: 'search_read',
            domain: buildDomain('planned'),
            context: { active_test: false },
        }).then(function (planned_activity) {
            self._rpc({
                model: 'mail.activity',
                method: 'search_read',
                domain: buildDomain('today'),
                context: { active_test: false },
            }).then(function (today_activity) {
                self._rpc({
                    model: 'mail.activity',
                    method: 'search_read',
                    domain: buildDomain('overdue'),
                    context: { active_test: false },
